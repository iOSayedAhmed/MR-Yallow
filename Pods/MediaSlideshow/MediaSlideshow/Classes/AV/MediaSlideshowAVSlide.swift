...
//            self.pausedOverlayView?.frame.size.height = 40
//            self.pausedOverlayView?.frame.size.width = 40
//

        self.transitionView = UIImageView()
        super.init(frame: .zero)
//            self.pausedOverlayView?.isUserInteractionEnabled = true
//        addGestureRecognizer(UITapGestureRecognizer(target: pausedOverlayView, action: #selector(didSingleTap)))
//            let tap = UITapGestureRecognizer(target: self, action: #selector(self.didSingleTap))
//            pausedOverlayView?.addGestureRecognizer(tap)
